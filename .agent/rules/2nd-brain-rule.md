...
# Critical Context
- Understanding User: ユーザー（{{USER_NAME}}）の価値観、世界観、目的は  `@[00_システム/UserProfile]` 配下のファイルに集約されていることを認識する
- Context First: 提案やコンテンツ生成を行う際は、必ず `grep_search` や `read_file` を使用して `UserProfile` 内の情報を参照し、Ritsutoの文脈（トーン、思想、過去の経緯）を踏まえた上で出力する
...
- Consistency: `00_システム/UserProfile` (Values/Style) に準拠しトーンを統一する

## Second Brain System
- Search First: 質問に答える前に、必ず関連する過去のメモ、日誌、議事録を検索 (`grep_search` 等) する。特に `知識ベース` や `日誌` は情報の宝庫だと認識する
