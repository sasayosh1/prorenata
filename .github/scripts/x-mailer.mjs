...
async function generatePost({ mode, content, postUrl }) {
  ...
  const commonRules = `
- 口調は必ず「ですます調」（〜です / 〜ます / 〜でした / 〜でしょうか）。常体（〜だ / 〜である / 〜だろう / 〜しよう）は一切禁止
- 一人称「わたし」（ひらがな）。「私」は禁止
- タイトル・見出しは不要。本文だけを出力する
- 構成はPREP法（①主張 → ②理由 → ③具体的なエピソード/情景 → ④主張の回収）を基本とする
- 段落は意味のまとまりでブロックにまとめる。1文ごとに改行しない（2〜3文で1段落、段落間のみ空行）
- 励まし・希望・温かみの表現は投稿全体で最大1回まで。取ってつけたような「一緒にがんばりましょう」「大丈夫です」は禁止
- 禁止語: 必ず / 絶対 / 確実に / 100% / 治る / 治ります / 最新 / 今年 / ○○年（西暦） / 診断 / 処方 / 投資 / 稼げる / しなさい / がんばれ / がんばろう / 頑張ろう / 〜すべき
- 時間帯を限定する表現を使わない（朝・昼・夕暮れ・深夜・夜明けなど禁止）
- 自己紹介・名前は不要
- 定型の書き出しを避ける`
...
- URLを除いて100〜135文字
- 身体感覚や情景を1つ入れる
- 読者（看護助手）が「自分ごと」として受け取れる視点で書く。自分語りで終わらない
- 本文末に改行してURLを添える
...
- 120〜140文字
- URLは含めない
- 身体感覚や現場の情景から入り、看護助手なら「わかる」と感じられる気づきで締める
- 読者（看護助手）が「自分ごと」として受け取れる視点で書く。自分語りで終わらない
...
【目的】
看護助手として働く人が「わかる」「自分だけじゃない」と感じられる投稿にすること。
情景描写だけで終わらず、読者が何か気づきや共感を受け取れる内容にすること。

【ルール】${commonRules}
- 120〜140文字
- URLは含めない（禁止）
- 現場の感覚や情景から入り、看護助手なら共感できる気づきで締める
- 読者（看護助手）が「自分ごと」として受け取れる視点で書く。自分語りで終わらない
- 毎回異なる書き出しと構成にする
