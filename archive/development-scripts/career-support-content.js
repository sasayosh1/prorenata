    ...
    for (const template of careerSupportTemplates) {
      ...
      try {
        ...
        const articleData = {
          ...
          body: [{_type: 'block', children: [{_type: 'span', text: 'Placeholder'}]}]
        };
        const article = await client.create(articleData);
        console.log(`✅ キャリア支援コンテンツ作成 (placeholder): ${template.title}`);

        const body = generateCareerSupportContent(template);
        await client.patch(article._id).set({body: body}).commit();
        console.log(`✅ キャリア支援コンテンツ作成 (body): ${template.title}`);
        createdCount++
