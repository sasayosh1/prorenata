...
⚠️ **注意**: このドキュメントは定期的に更新されていない可能性があります。最新情報は以下を参照してください：
- 詳細ルール・最近の変更: [CLAUDE.md](./CLAUDE.md)
- 記事作成ガイド: [ARTICLE_GUIDE.md](./ARTICLE_GUIDE.md)
- 運用・監視ルール: [RULES.md](./RULES.md)
...
- 自動生成・メンテナンス系スクリプトは、文章を生成・加筆する際に `docs/ARTICLE_GENERATION_STRATEGY.md` の「キャラクターボイス指針（話者ルール）」を反映する。
- 一人称は「わたし」固定（「私」は使わない）。
- 文章内で自分の名前を出さない（例: 「セラが」「セラの」「白崎セラです」などは禁止）。
- タイトル/本文で「看護助手の私が教える、」のような肩書き主張は入れない。
- 断定的な表現を避け、現実的な助言を添える。
