...
// 大幅に簡略化されたMarkdownからPortable Textへの変換関数
function simplifiedMarkdownToPortableText(markdown) {
  const blocks = [];
    ...
    lines.forEach(line => {
      ...
      if (trimmed.startsWith('## ')) {
        blocks.push({
          ...
          _type: 'block',
          style: 'h2',
          children: [{ _key: key(), _type: 'span', text: trimmed.substring(3) }]
        });
        ...
        blocks.push({
          ...
          _type: 'block',
          style: 'h3',
          children: [{ _key: key(), _type: 'span', text: trimmed.substring(4) }]
        });
      } else if (trimmed.startsWith('#### ')) {
        blocks.push({
          _key: key(),
          _type: 'block',
          style: 'h4',
          children: [{ _key: key(), _type: 'span', text: trimmed.substring(5) }]
        });
      } else if (trimmed.startsWith('- ')) {
        blocks.push({
          _key: key(),
          ...
          style: 'normal',
          listItem: 'bullet',
          children: [{ _key: key(), _type: 'span', text: trimmed.substring(2) }]
        });
      } else if (trimmed) {
        // 太字処理を簡略化
        ...
        blocks.push({
          ...
          _type: 'block',
          style: 'normal',
          children: [{ _key: key(), _type: 'span', text: text }]
        });
      }
    });
  });
...
// テスト実行
console.log('=== 簡略化版変換テスト ===');
const simplifiedBody = simplifiedMarkdownToPortableText(newContent);

console.log('簡略化後のブロック数:', simplifiedBody.length);
console.log('簡略化後のJSON文字数:', JSON.stringify(simplifiedBody, null, 2).length);
...
fs.readFile(articlesFilePath, 'utf8', (err, data) => {
  ...
  try {
    ...
    // 簡略化されたPortable Textに変換
    const portableTextBody = simplifiedMarkdownToPortableText(newContent);
    ...
    fs.writeFile(articlesFilePath, JSON.stringify(articles, null, 2), 'utf8', (err) => {
      ...
      console.log('Article updated with simplified conversion.');
    });
  } catch (parseErr) {
