...
- 直近の記事が「同じテンプレ」「似た構文」にならないように、生成後にタイトルを自動で軽く整形する（一覧の視認性とSEOの過度なカニバ対策）。
- タイトルは次を守る:
  - `【】` の括弧装飾は使わない（一覧で同じ見た目になりやすい）。
  - 末尾を `…` / `...` で終えない（途中省略で終わらせない）。
  - 括弧付き補足（例: （...））は使わない。
  - `徹底解説` / `完全ガイド` の連発を避け、`要点整理` / `状況整理` などへ言い換える。
  - 直近のタイトル群と類似度が高い場合は、別角度のタイトル候補へ差し替える。
- URL（slug）は変えず、タイトルのみを調整する。
...
**戦略**: 狙って書くよりも **"全体像を整理するトップ構造"として作る**

**具体例**:
```
「看護助手の全体像｜仕事内容・資格・転職・給料の整理」
    ↓
...
### 1. 記事構成と文字数の方針（ProReNata Blog / Mode A）

**最優先事項: 読者の悩み解決と情報の網羅性（Comprehensiveness）**

- **文字数に関するルール**:
  - **上限（キャップ）は設けない**。読者に納得・満足してもらうために必要な情報はすべて含める。
  - 文字数は「網羅的に、丁寧に書いた結果」として増えるものであり、短くすることを目的とした「引き算」や「文字数制限」は行わない。
  - 公的データ（厚労省、日本看護協会等）や根拠となる事実を惜しまず盛り込み、説得力のある「芯の通った」解説を目指す。
- **目安（最小文字数）**:
  - ロングテール: 最低 1500〜2000文字程度
  - ミドルテール: 最低 2500〜3000文字程度
  - ショートテール（ガイド記事）: 3000文字〜
- **ユーザビリティ**: 文字数が多くなっても、見出し構造や箇条書きを活用し、読みやすさと「情報の充足度」を両立させる。

### 2. E-E-A-T強化の実装

すべての記事に以下を含める:

- ✅ **網羅的な解説**: 読者の不安が解消されるまで丁寧に説明する。
- ✅ **0.5% のナラティブ要素**: 現場の空気感や血の通った温かみを加えるスパイス。
- ✅ 具体的な経験・事例
- ✅ 実践的なアドバイス
- ✅ 数字・データ（根拠の提示を最優先）
