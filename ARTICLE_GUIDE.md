...
- **配置**: H2/H3の直下に「参考: …」を置かないでください。必ず説明本文を書いてから出典を配置し、複数の参考リンクや内部リンクを連続させないように本文で区切ります。メンテナンススクリプトも同ルールで再配置します。
...
- **カテゴリ別ルール**:
  - 転職カテゴリの記事に直接案件リンク（ヒューマンライフケア等）を挿入せず、比較記事 `nursing-assistant-compare-services-perspective` への内部リンクを必ず1つ置く。
  - 退職カテゴリの記事も同様に、比較記事 `comparison-of-three-resignation-agencies` へ内部リンク誘導する。
  - これらのカテゴリでは `humanlifecare` / `kaigobatake` / `renewcare` など就職・転職系 `affiliateEmbed` を本文に残さないこと。メンテナンススクリプトが近接する該当ブロックを自動で除去し、代わりに比較記事リンクを「まとめ」直前へ再挿入します。
  - アイテム系（ナースリー / Amazon / 楽天）は、薄いブルーのカード内で訴求文＋[PR]リンクの構成に統一する。Amazon/Rakutenは2リンクセットで必ず並べる。
- **配置ルール**:
  - サービス案件の `affiliateEmbed` は、基本的に「まとめ」本文の直後（免責事項の前）へ配置する。まとめ後に参考リンクを置くことは不可。
...
#### 「〇〇選」形式の例外
「おすすめ転職サービス3選」のように、複数の選択肢を比較提示するセクションでは、各選択肢（H3見出しごと）にアフィリエイトリンクを設置することが許可されます。ただし、以下の条件を満たす必要があります。
