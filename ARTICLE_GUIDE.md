...
- **配置ルール**:
  - 原則としてサービス案件の `affiliateEmbed` は「まとめ」本文の直後（免責・参考リンクより前）に配置する。
  - 4000字を超える長尺記事で、該当サービスの詳細セクションが存在する場合のみ、中盤の関連H2直後にも1つ追加してよい（合計2箇所まで）。参考リンクとの連続配置は可。
  - 中盤配置時も薄いブルー背景のCTAとし、まとめ後の配置は必須。
