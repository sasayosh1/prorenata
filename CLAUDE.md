...
59. 📝 **H2/H3セクション末尾の箇条書き一括修正** (2025-11-25)
   - **背景**: ユーザーから「セクションによって箇条書きで終わっている」との指摘
   - **検出結果**:
     - H2セクション問題: 12記事
     - H3セクション問題: 157記事
     - 合計修正対象: 169件
   - **修正内容**:
     - 箇条書きの後にフォロー文を追加
     - セクション構成を「主張 → 理由（箇条書き） → フォロー」に統一
   - **安全対策** (RULES.md セクション7に従って実施):
     - ✅ 修正対象フィールドを明確化: body内のH2/H3セクション末尾のみ
     - ✅ 修正対象外フィールドを明確化: title, excerpt, categories, tags等は一切触らない
     - ✅ 収益最重要記事（PROTECTED_REVENUE_SLUGS）をスキップ
     - ✅ maintenanceLocked: trueをスキップ
     - ✅ ドライラン→少数テスト（3件）→全件実行の順守
   - **実行結果**:
     - 処理した記事: 163件
     - 修正した記事: 77件
     - 修正なし: 86件
   - **スクリプト**: `scripts/tmp/fix-section-endings.js`
   - **効果**: すべてのH2/H3セクションがRULES.mdの本文構成ルールに準拠

58. 🚨 **重大インシデント: 自己参照リンクによるユーザビリティ違反** (2025-11-25)
   - **問題発見**: 転職サービス比較記事（`nursing-assistant-compare-services-perspective`）に自分自身への内部リンクが2箇所存在
     - まとめセクション（H2）直後に自己参照リンク
     - 記事中盤にも同じ自己参照リンク
   - **影響**: ユーザーが同じページ内でループし、離脱率上昇・UX低下
   - **原因**: 復元スクリプトで内部リンクを追加する際、リンク先が自分自身であることをチェックしていなかった
   - **緊急対応**:
     - 自己参照リンクを2件削除（Block 31, 64）
     - 退職代行比較記事もチェック（問題なし）
   - **再発防止策**:
     - RULES.mdに「自己参照リンク禁止」ルールを追加
     - 比較記事自体が目的地なので、自分自身への内部リンクは不要
     - 内部リンク追加時は必ずリンク先が現在の記事と異なることを確認
   - **コミット**: e7e4459
   - **重要度**: ユーザビリティに直接影響する重大インシデント

57. 📊 **タイトル・冒頭文のSEO最適化（19件一括修正）** (2025-11-25)
   - **問題発見**: 【】括弧使用、タイトル長超過、冒頭挨拶文が残っている記事が複数存在
   - **ルール追加**: CLAUDE.mdの記事品質基準に以下を追加
     - タイトル最適化: 【】括弧や過度な装飾は使用しない。キーワードを自然に配置し、30〜40文字に収める
     - 冒頭文: 挨拶文・自己紹介（「〇歳の看護助手として...」など）は使用せず、すぐに本題に入る
   - **修正内容**:
     - 19件の記事タイトルを最適化（23〜33文字に短縮）
     - 【】括弧を全て削除（10件）
     - タイトル長超過を修正（11件）
     - 冒頭挨拶文を削除（2件）
   - **新規スクリプト**:
     - `scripts/optimize-titles-and-intros.js`: タイトル・冒頭文一括最適化ツール
   - **効果**: 検索結果でタイトルが完全表示されるようになり、クリック率向上が期待できる。記事冒頭がすぐに本題に入るため、読者の離脱率低下も見込める
