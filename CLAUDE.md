...
74. 📊 **図解・挿絵定義書の策定とビジュアル資産の整理** (2026-03-17)
   - **背景**: サイト内の図解や挿絵の一貫性を保つため、設計基準を明確化
   - **実施内容**:
     1. **定義書の新規作成**:
        - `00_ProReNata図解定義書.md`: インフォグラフィックス、カラーパレット、デザイン原則（Rounded-2xl, Shadow-xl）を定義。
        - `00_ProReNata挿絵定義書.md`: 白崎セラのチビキャラ（2-2.5頭身）、表情、ポーズ、色彩基準を定義。
     2. **フォルダ構成の整理**:
        - `画像/挿絵` フォルダを `画像/ProReNata/図解/挿絵` へ移動し、ビジュアル資産を一元管理。
     3. **命名規則の適用**:
        - 既存の挿絵ファイルを新規則（`sera-chibi-verb-situation.png`）に基づきリネーム。
     4. **ブランド指針への統合**:
        - `11_ブランドビジュアル指針(Brand_Visual_Guidelines).md` に各定義書へのリンクを追加。
   - **効果**: 今後のビジュアル制作における「白崎セラらしさ」と「情報の伝わりやすさ」の両立を担保。
   - **作業ブランチ**: `main`
...
---
   ...
   - **修正ファイル**:
     1. `scripts/moshimo-affiliate-links.js`: 情報を完全に記載更新
     2. `scripts/remove-affiliate-links.js`: 削除対象リストからガイアを除外（コメント化）
     3. `PROJECT_STATUS.md`: 復活記録を追記
   - **効果**: 退職代行3サービス（みやび、即ヤメ、ガイア）の選択肢が全揃い

73. 💰 **ナースリーアフィリエイトコードを新しいpidに一括更新** (2025-11-29)
   - **背景**: ユーザーから「リンクエラーになっているものが多数ある」との報告。ValueCommerce管理画面で新しいナースリー案件コードを確認
   - **問題**: 73件の記事で古いpid（892289712）が使用されており、「このリンク先は現在広告配信準備中です」エラーが発生
   - **修正内容**:
     - `scripts/moshimo-affiliate-links.js`: ナースリー案件を新コードに更新
       - 旧: sid=3755453 or 3757192, pid=892289712
       - 新: sid=3757192, pid=892328264
     - `scripts/tmp/fix-nursery-links.js`: 一括更新スクリプト作成
       - 複数の古いコードパターン（2種類）を検出・置換
       - affiliateEmbedブロックとmarkDefs内のリンクの両方を処理
       - 73件すべての記事を自動更新
     - `.env.local`: SANITY_WRITE_TOKEN更新（期限切れトークンの再発行）
   - **実行結果**:
     - 修正したリンク数: 73件
     - 更新した記事数: 73件
     - すべてのナースリーリンクが新しいアフィリエイトコードに更新完了
   - **効果**:
     - ValueCommerce管理画面で正確なクリック数・成果計測が可能に
     - ユーザーがナースリーLPに正しく遷移できるようになった
     - 収益機会の損失を防止
...
71. 🚨 **【重大インシデント】メディカルクイズのリセット時間改変の復旧** (2025-11-26)
   ...
   - **問題**: 現在の実装では `new Date().toISOString()` を使用していたため、UTC基準で日付を判定し、日本時間の朝9時にリセットされていた
   - **原因**: 作成時は `toLocaleDateString('sv-SE', { timeZone: 'Asia/Tokyo' })` だったが、メンテナンススクリプトか何かで `toISOString()` に改変された可能性
   - **修正内容**:
     - `new Date().toISOString().split('T')[0]` → `new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Tokyo' })`
     - 日本時間（JST）で日付を取得するように変更
     - `sv-SE`（スウェーデン）ロケールを使用することで、YYYY-MM-DD形式を維持
   - **効果**:
     - リセット時刻: JST 9:00 → **JST 0:00（深夜）**
     - ユーザーは日本時間の深夜0時から新しい10問に挑戦可能
     - より自然な日次リセットタイミングを実現
   - **修正ファイル**: `src/components/MedicalTermQuiz.tsx:80`
   - **再発防止策**:
     - CLAUDE.mdに「時間設定・タイムゾーン設定の改変は絶対禁止」ルール追加
     - CLAUDE.mdに「指定された作業以外のコード変更は絶対禁止」ルール追加
     - 今後は時間に関する設定を変更する必要がある場合は、必ず事前にユーザーに確認すること

70. 🎮 **医療用語記事にメディカルクイズへのリンク追加** (2025-11-25)
   - **背景**: ユーザーから「いい感じの場所にメディカルクイズを配置してください」との指示、その後「内部リンクの背景色をつけてください」「新しいタブで開くようにしてください」との追加指示
   - **対象記事**: `nursing-assistant-medical-terms`（看護助手に必要な医療用語100選｜現場でよく使われる略語も解説）
   - **配置場所**: Block 72（「医療用語を効率よく覚えるための学習法」セクションの後、「まとめ」セクションの前）
   - **修正内容**:
     - メディカルクイズへの内部リンクを追加
     - リンクテキスト: 「医療用語を覚えたら、メディカルクイズで理解度をチェックしてみましょう。クイズ形式で楽しく復習できます。」
     - リンク先: `/quiz`（内部リンク形式、当初は `https://prorenata.jp/quiz` だったが `/quiz` に修正）
     - 内部リンク背景色: ピンク色（`bg-rose-50/80`）が自動適用（`PortableTextComponents.tsx` の `CustomParagraph` により）
     - 新しいタブで開く: `openInNewTab: true` を設定
   - **安全対策**:
     - ✅ ドライラン→実行の順守
     - ✅ 修正対象: body内のBlock 72への新規ブロック追加、hrefの修正、openInNewTabの設定のみ
     - ✅ 修正後の確認を実施
   - **実行結果**:
     - メディカルクイズリンク追加: 1件
     - 内部リンク形式への修正: 1件
     - 新しいタブで開く設定: 1件
     - 総ブロック数: 80 → 81
     - Block 72: メディカルクイズリンク（新規、内部リンク形式、新しいタブで開く）
     - Block 73: まとめ（旧Block 72）
     - 保存完了: ✅
...
74. 💰 **弁護士法人ガイア法律事務所のアフィリエイト復活** (2025-12-03)
   ...
   - **修正内容**:
     - 修正前: ValueCommerce sid=3515026, pid=892170743（間違ったコード）
     - 修正後: ValueCommerce sid=3757192, pid=892314166（正しいコード）
     - まとめセクション後のアフィリエイトブロック（Block 51）と同じコードに統一
   - **安全対策**:
     - ⚠️ 収益最重要記事（PROTECTED_REVENUE_SLUGS）だが、アフィリエイトコードの誤りは収益に直結する重大問題のため修正
     - ✅ ドライラン→実行の順守
     - ✅ 修正対象: body内のBlock 18のmarkDefs[0].hrefのみ
     - ✅ 修正後の確認を実施
   - **実行結果**:
     - アフィリエイトコード修正: 1件
     - 保存完了: ✅
     - 修正後確認: ✅（正しいコードに変更されたことを確認）
   - **スクリプト**: `scripts/tmp/fix-miyabi-affiliate.js`
   - **効果**: 3サービス比較セクションとまとめセクション後で同じアフィリエイトコードが使用され、収益計測が正確に

68. 🔗 **給料記事に転職サービス３社比較への内部リンク追加** (2025-11-25)
   - **背景**: ユーザーから「より条件の良い職場へ転職するのセクションの終わりに、転職３選の内部リンクを貼ってください」との指示
   - **対象記事**: `nursing-assistant-average-salary-data`（看護助手の平均給料はいくら？最新データまとめ）
   - **対象セクション**: H3「より条件の良い職場へ転職する」（Block 38）
   - **修正内容**:
     - Block 40（空段落）を削除
     - Block 41（ヒューマンライフケア誘導）の後に内部リンクを追加
     - リンクテキスト: 「詳しくは、看護助手の転職サービス３社を "看護助手の視点だけ" で比較をご覧ください。」
     - リンク先: `/posts/nursing-assistant-compare-services-perspective`
   - **安全対策** (RULES.md セクション7に従って実施):
     - ✅ 修正対象フィールドを明確化: body内の特定ブロックのみ
     - ✅ 修正対象外フィールドを明確化: title, excerpt, categories, tags等は一切触らない
     - ✅ 収益最重要記事（PROTECTED_REVENUE_SLUGS）チェック
     - ✅ maintenanceLocked: trueチェック
     - ✅ ドライラン→実行の順守
   - **実行結果**:
     - 空段落削除: 1件
     - 内部リンク追加: 1件
     - 保存完了: ✅
   - **スクリプト**: `scripts/tmp/add-comparison-link.js`
   - **効果**: 転職を検討している読者が比較記事へスムーズに遷移できる導線を確保

67. 📝 **H2/H3セクション末尾の箇条書き一括修正** (2025-11-25)
...
66. 🚨 **重大インシデント: 自己参照リンクによるユーザビリティ違反** (2025-11-25)
...
65. 📊 **タイトル・冒頭文のSEO最適化（19件一括修正）** (2025-11-25)
...
64. 🔧 **退職関連記事の不適切アフィリエイトリンク削除と内部リンク追加** (2025-11-25)
...
63. 🎨 **アイテムアフィリエイトの訴求文を記事内容に応じて動的生成** (2025-11-25)
...
62. 📝 **アイテム記事作成とMeta Description文字数ルール追加** (2025-11-24)
...
57. 🎯 **PRカード表示と記事セクション構成の整理** (2025-11-23)
...
58. 🔗 **退職代行比較記事のアフィリエイトリンク再設定** (2025-11-24)
...
59. 🔁 **アフィリエイト配置の全量リセットと標準化** (2025-11-24)
...
60. 🔧 **A8.net「かいご畑」アフィリエイトコードの完全統一** (2025-11-24)
...
61. 🔗 **転職・退職記事のアフィリエイトリンク修正とルール追加** (2025-11-24)
...
56. ♻️ **制度アップデート体制とナビゲーション改善** (2025-11-21)
...
54. 🎮 **メディカルクイズの重複問題修正と名称変更** (2025-11-19)
...
53. 🔧 **メンテナンスチェックの毎日実行化** (2025-11-18)
...
1. 👤 **白崎セラのプロフィールページ実装とキャラクター設定拡充** (2025-10-26)
...
3. ⏰ **ワークフロー実行スケジュール最適化** (2025-10-26)
   - ワークフロー実行時間を整理・最適化
...
5. 🔑 **ローカル環境の Sanity トークン設定** (2025-10-26)
...
6. ✨ **メンテナンススクリプトの白崎セラ口調対応** (2025-10-26)
...
7. 🔗 **アフィリエイトリンクチェックの最適化** (2025-10-26)
...
9. 🚨 **メンテナンススクリプトが実行されていなかった重大バグ修正** (2025-10-26)
   - **発覚した問題**:
...
10. 🗂️ **不適切な「テクノロジー」カテゴリ問題を修正** (2025-10-26)
   - **発覚した問題**:
...
11. 📊 **カテゴリ統合とテール戦略の自動バランス調整機能実装** (2025-10-27)
   - **カテゴリ統合（16→14種類）**:
...
12. 🧹 **記事冒頭の不要な挨拶文を自動削除する機能を追加** (2025-10-27)
...
13. 🐛 **挨拶文削除機能のnullセーフティエラーを修正** (2025-10-27)
...
14. 🐛 **GitHub Actions メンテナンスワークフローの権限エラー修正** (2025-10-27)
...
15. 💰 **Gemini APIコスト削減（▼18%削減）** (2025-10-28)
...
16. 🚨 **緊急修正：Gemini Proモデルへのフォールバック防止（▼90-95%削減）** (2025-10-28)
   - **重大問題の発見**:
...
19. 🚨 **Gemini API Pro禁止ルールを全プロジェクトに拡大** (2025-10-29)
...
20. 🔗 **アフィリエイトリンク・出典リンク自動挿入機能を実装（収益化・YMYL対策）** (2025-10-29)
   - **重大な問題を発見・解決**:
...
21. 🔧 **Slug生成ルールの改善（SEO対策）** (2025-10-29)
...
22. ✨ **Gemini API不使用時の高品質フォールバック機能を実装** (2025-10-29)
...
23. 🔧 **白崎セラのペルソナ定義モジュール化＆GA4設定** (2025-11-05)
   - **ペルソナ定義の共通モジュール化**:
...
24. 🧩 **「まとめ」セクション監視＆復旧フローを追加** (2025-11-07)
...
25. ⚖️ **免責事項ブロックを「まとめ」直後に固定** (2025-11-08)
...
26. 📝 **H2見出しから「セラ」を自動排除し、一人称を「わたし」に統一** (2025-11-08)
...
27. 💼 **アフィリエイト枠は最大2件（Amazon/Rakuten除く）＋枠内CTAに統一** (2025-11-08)
...
28. 📚 **一次資料のホワイトリスト化とリンク検証** (2025-11-08)
...
29. 🗂️ **カテゴリ語彙の正規化とStudio同期コマンド導入** (2025-11-09)
   - **背景**:
...
30. 🔧 **ポート番号を明示的に設定（toyamablogとの競合解消）** (2025-11-09)
...
31. 📚 **医療用語データベースとメンテナンススクリプト追加** (2025-11-09)
...
32. 🔁 **全160記事のsanitize再実行（Flash Lite固定＋リンク強制挿入）** (2025-11-09)
   - **背景**:
...
34. 🛠️ **Sanity StudioでのMissing keys / Unknown field警告を根絶** (2025-11-10)
   - **背景**:
...
35. 🛍️ **ナースリー案件の無制限化とアフィリエイト間隔ルール追加** (2025-11-10)
...
36. 🔐 **内部限定記事フラグ（internalOnly）とNoIndex対応** (2025-11-10)
...
37. 🔁 **退職記事は内部比較記事へ誘導＆退職代行ASPを除外** (2025-11-10)
...
33. 🐛 **GitHub Actions maintenance_check エラー修正（2回の修正）** (2025-11-10)
...
38. 🐛 **GitHub Actions メンテナンスワークフローのtotalAffiliateLinksInserted変数未定義エラーを修正** (2025-11-13)
...
39. 🐛 **メンテナンススクリプトの正規表現構文エラーを修正** (2025-11-13)
...
40. 🐛 **module.exportsの後の孤立したコードを削除** (2025-11-13)
...
41. 🐛 **sanitizeAllBodies関数にinternalLinkTitleMapの定義を追加** (2025-11-13)
