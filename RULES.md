...
- **本文構成ルール**
  ...
  - セクションの末尾を箇条書きで終えない。箇条書き後は安心して読み終えられるために、情報を整理する文章で締める。
  - 共感と理解を深めるために、状況を整理する文章で補足する。
  - 箇条書きのみで構成されたセクションは禁止。必ず前後に本文を挿入する。
  - 本文でチェックリスト/ToDo/STEP形式や「次にやること」「最後に確認」などの行動リスト誘導は作らない。
  - 箇条書きは説明補助としてのみ使用し、語尾に「〜しましょう/してください」等の行動指示を置かない。
