...
- **適用範囲**: **すべてのスクリプトに適用（メンテナンススクリプトを含む）**
  - 週次メンテナンス（`scripts/maintenance.js all`）
  - 個別修正スクリプト（`scripts/fix-*.js`等）
  - 新規作成スクリプト
- **絶対厳守事項**:
  1. **指示された作業以外の部分は絶対に触らない**
     - メンテナンススクリプトであっても、指示されていない修正は一切行わない
     - 例: H3セクション末尾の修正を指示された場合、title、excerpt、categories、tags等は一切触らない
...
- **メンテナンススクリプトの特別ルール**:
  - `scripts/maintenance.js` の `autoFixMetadata()` や `sanitizeAllBodies()` は多数のフィールドを自動修正する
  - これらの機能を使う際は、どのフィールドが修正されるか事前に確認する
  - 指示されていない修正機能は必ず無効化する
  - 週次自動実行（GitHub Actions）でも同様のルールを適用
