# ProReNata プロジェクト現状報告 (2026-03-23 更新)

## 最新の重要な変更

### 💰 AIツールの費用最適化と安全機能の導入 ✅
- **日時**: 2026-03-23
- **内容**: Claude Code と Codex CLI の導入に伴う高額課金リスクを低減。
- **実施内容**:
  1. **コンテキスト軽量化**: `CLAUDE.md` の過去ログをアーカイブし、入力トークンを約70%削減。
  2. **安全装置**: `./tools/ai` コマンドに実行前の確認プロンプトを実装。
  3. **ガイドライン更新**: `09_AIツール活用指針(AI_Tool_Strategy).md` にコスト管理ルールを追記。
- **重要度**: 運用コストの無料枠内維持。

### 🎨 ProReNata LINEスタンプ第1弾 申請完了 ✅
- **日時**: 2026-03-20
- **内容**: 「看護助手の本音 - 白崎セラ」LINEスタンプ（全8個〜）をLINE Creators Marketへ申請。
- **ステータス**: 審査待ち
- **価格**: 120円
- **ファイル**: [01_申請記録.md](file:///Users/sasakiyoshimasa/prorenata/LINEスタンプ/01_申請記録.md)
- **重要度**: ブランド認知度向上とファン層の拡大。キャラクター資産の有効活用。

### 📊 図解・挿絵の定義化とビジュアル資産の整理 ✅
- **日時**: 2026-03-17
- **内容**: 図解（インフォグラフィックス）と挿絵（チビキャラ）の品質基準をマニュアル化。資産の整理と命名規則の統一を実施。
- **成果物**: 
...
- **整理**: `画像/挿絵` を `画像/ProReNata/図解/挿絵` へ移動し、命名規則を適用。
- **重要度**: ブランドの一貫性維持と、外部委託・AI生成時の指示精度向上。

### 📊 記事全件の技術的監査と健康診断の実施 ✅
- **日時**: 2026-03-06
- **内容**: ProReNataの全184記事を対象に、文字数、リンク数、アフィリエイト密度、禁止語句の技術的監査を自動実行。
- **結果**: [audit_report.md](file:///Users/sasakiyoshimasa/prorenata/audit_report.md) に詳細を記録。多くの記事で「内部リンク不足」や「低文字数」が判明。
- **重要度**: サイト全体の品質底上げとSEO・回遊性の改善。

### 🧠 NotebookLM 連携用ソースの生成 ✅
- **日時**: 2026-03-06
- **内容**: 過去の全記事本文とマスターコンテキストを統合し、NotebookLMに読み込ませるための巨大なソースファイルを生成。
- **ファイル**: [prorenata_notebooklm_source.md](file:///Users/sasakiyoshimasa/prorenata/prorenata_notebooklm_source.md)
- **目的**: 記事間の論理的矛盾の発見、セラのキャラクター一貫性のチェック、および「先輩」などの設定深掘り。
- **重要度**: 世界観の解像度向上とブランド資産の整理。

### 🐦 X（Twitter）投稿生成のロジック強化と完全自動化 ✅
- **日時**: 2026-03-06
- **内容**: PV上位30記事からのランダム選出ロジックを実装。2日に1回（AM6:00 JST）の自動生成ワークフローを構築。
- **ファイル**: `scripts/generate-x-posts-daily.cjs`, `.github/workflows/x-post-bi-daily.yml`
- **重要度**: 運用コスト削減と需要の高いコンテンツの重点訴求。

### 🛠️ Obsidian 環境の最適化と AI ログ資産化システムの導入 ✅
- **日時**: 2026-02-03
- **内容**: 開発効率と知識蓄積を両立させるため、Obsidian の構成を本質的に改善し、AI作業の自動記録を開始。
- **実施内容**:
  1. **Vaultの実体移行**: iCloud からローカル (`~/Obsidian`) へ実体を移動し、同期の不安定さを解消。
  2. **シンボリックリンク構成**: `prorenata` / `toyamablog` リポジトリの実体をホーム直下に置き、Obsidian 内からはリンクで見せる「エンジニア最適解」の構成。
  3. **AI 作業ログの自動化**: `99_System/AI_Logs/` に AI の活動を自動記録する仕組みを導入。
- **重要度**: 開発環境の安定化およびナレッジの資産化。

### 🛑 不要な自動ワークフローの完全停止 ✅
- **日時**: 2026-02-03
- **内容**: 運用の見直しに伴い、一部の GitHub Actions を完全に停止。
- **対象**:
  - `x-mailer.yml` (Xメーラー): 定期配信を停止（コメントアウト）。
  - `weekly-quiz-generation.yml` (クイズ自動生成): 定期生成を停止。
- **ステータス**: GitHub リポジトリ (`main` ブランチ) へ反映済み。

---

### 💰 弁護士法人ガイア法律事務所のアフィリエイト復活 ✅
- **日時**: 2025-12-03
- **内容**: 弁護士法人ガイア法律事務所（退職代行サービス）のアフィリエイトリンクを復活・情報更新
- **更新内容**:
  1. **アフィリエイト情報の完全化**
     - 報酬額: 14,000円
  ...
  2. **マスターファイル更新**: `scripts/moshimo-affiliate-links.js`
     - 報酬額、条件、ネットワーク、公式URLを追加記載
     - active: true で有効化
  3. **対象カテゴリ**: 退職代行（みやび、即ヤメと並行運用）
- **重要度**: 退職代行サービスの選択肢拡充・収益導線強化
- **効果**: 「comparison-of-three-resignation-agencies」記事での案件提示幅拡大

### 🆕 介護美容スクール紹介記事を公開 ✅
- **日時**: 2025-11-27
- **内容**: 「介護美容で笑顔を増やす学び方とスクール活用術」（slug: `nursing-assistant-care-guide-toolbox`）をSanityへ追加。
- **ポイント**:
...
- **実施内容**:
  1. **マスターファイルから削除**
     - アルバトロス転職（a_id: 5211244） - **削除済み**
  2. **記事から削除**
     - 「全国看護助手給与実態調査2024」からアルバトロス転職リンク削除（1件）
  3. **現在の登録アフィリエイト数**: 10種類（最新反映済み）
     - 就職・転職
       - ヒューマンライフケア｜レントラックス｜17,600円｜条件: 申込定額｜リンク: https://jobs.human-lifecare.jp/｜コード: `<a href="https://www.rentracks.jp/adx/r.html?idx=0.71551.371865.8943.12704&dna=148900" rel="nofollow noopener" target="_blank">ヒューマンライフケア</a><img src="https://www.rentracks.jp/adx/p.gifx?idx=0.71551.371865.8943.12704&dna=148900" height="1" width="1" border="0">`
       - リニューケア｜もしも a_id=5207862｜7,000円｜条件: 新規会員登録（関西特化）｜リンク: https://renew-care.com/register_lp
       - かいご畑｜A8.net a8mat=2ZTT9A｜6,000円｜条件: 新規人材サービス登録｜リンク: https://kaigobatake.jp/lp/af/other/rwd/lp12/
     - 資格
       - 介護美容研究所｜A8.net a8mat=3BQLBJ+BSICCY+4LBY+5YJRM｜1,500円｜条件: 新規資料請求｜リンク: https://academybc.jp/lp6s/
     - アイテム
       - ナースリー｜ValueCommerce sid=3755453, pid=892289712｜10.47%（9.52%）｜条件: 商品購入｜リンク: https://www.nursery.co.jp/
       - Amazon｜もしも a_id=5211352｜報酬: カテゴリによる｜条件: 商品購入｜リンク: https://www.amazon.co.jp/
       - 楽天市場｜もしも a_id=5207851｜2%｜条件: 商品購入｜リンク: https://www.rakuten.co.jp/
     - 退職代行
       - 弁護士法人みやび｜ValueCommerce sid=3515026, pid=892170743｜16,500円｜条件: 商品購入｜リンク: https://taishoku-service.com/vc/
       - 退職代行 即ヤメ｜もしも a_id=5211257｜15,000円｜条件: Webからの退職代行依頼｜リンク: https://taishokudaikou-sokuyame.com/
       - 弁護士法人ガイア法律事務所｜もしも a_id=5211256｜14,000円｜条件: LINE/メール相談完了｜リンク: https://www.gaia-law-office.jp/taisyoku/
...
### 使用中のアフィリエイトコード（2025-12-03時点）
1. **ヒューマンライフケア**: `a_id=5207863` (もしも) - 報酬15,385円
2. **かいご畑**: `a8mat=2ZTT9A` (A8.net) - 報酬6,000円
3. **リニューケア**: `a_id=5207862` (もしも) - 報酬7,000円
4. **介護美容研究所**: A8.net - 2025-11-27追加
5. **ナースリー**: `sid=3757192, pid=892328264` (ValueCommerce) - 2025-11-29更新
6. **Amazon**: `a_id=5211352` (もしも) - 報酬2.0%
7. **楽天市場**: `a_id=5207851` (もしも) - 報酬2%
8. **弁護士法人みやび**: `sid=3757192, pid=892314166` (ValueCommerce) - 報酬16,500円
9. **退職代行 即ヤメ**: `a_id=5211257` (もしも) - 報酬15,000円
10. **弁護士法人ガイア法律事務所**: `a_id=5211256` (もしも) - 報酬14,000円
...
12. ⚠️ リンク背景色表示問題の解決
13. 📋 画像表示機能の動作確認
14. 📋 **[持ち越し]** 監査レポートに基づく記事のリファクタリング（内部リンク追加・低品質記事の統合）
15. 📋 **[持ち越し]** NotebookLMによる世界観の監査と「先輩」「薬」等の設定具体化
16. ✅ LINEスタンプ制作 (Ver.1) 申請完了
17. 📋 **[予定]** LINEスタンプ第2弾の企画 (No.22〜)
