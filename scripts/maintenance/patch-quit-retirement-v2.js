...
const bodyBlocks = [
    ...
    createBlock('信頼できる人に相談：同僚、家族、労組、専門窓口。感情の整理と第三者の視点があるだけで、決断の重さが少し軽くなるでしょう。', 'normal', 'bullet'),
    createLinkBlock('参考：', 'おすすめ退職代行3社比較', '/posts/comparison-of-three-resignation-agencies'),
    ...
    createBlock('テンプレや相談を受けつつ、自分名義で進める方法。コストを抑えたいが手順に不安がある人向けです。メッセージ文例が欲しいときにも役立ちます。', 'normal'),

    createBlock('比較の目安', 'h4'),
    // Table representation via text blocks
    createBlock('【退職方法の比較表】', 'normal'),
    createBlock('・自分で伝える：0円 / 即日性△ / 法的安心度△ (向いている人：関係良好・交渉慣れ)', 'normal', 'bullet'),
    createBlock('・退職代行：数万円 / 即日性◎ / 法的安心度○〜◎ (向いている人：即日・不仲・ハラスメント)', 'normal', 'bullet'),
    createBlock('・セルフ退職：低〜中コスト / 即日性○ / 安心度△〜○ (向いている人：コスト抑えたい・要相談)', 'normal', 'bullet'),
    createBlock('それぞれの方法で「いまの自分にとって何が一番ラクか」を基準に選んで大丈夫です。', 'normal'),

    createBlock('退職代行を使う場合・使わない場合の差分', 'h2'),
    createBlock('使わない場合、上司との面談や引き継ぎの段取りを自分で行う必要があります。日程調整のストレスはあるものの、費用を抑えることが可能です。関係が悪くない職場なら、円満退職しやすいメリットもあります。', 'normal'),
    createBlock('使う場合は連絡や交渉を任せられるので、心理的・時間的な負担が大きく減ります。特にハラスメントや強い引き止めがある現場では、第三者が間に入ることで安全に進められます。', 'normal'),
    ...
    createBlock('弁護士が直接対応し、未払賃金やハラスメント交渉も相談可。医療・介護の実績が多く、書類や有休消化にも配慮。非弁リスクを完全に避けたい人に向きます。', 'normal'),
    createBlock('まずは公式サイトで提供されている条件をチェックしてみてください。', 'normal'),

    createBlock('退職代行 即ヤメ（スピード・24時間相談）', 'h3'),
    createBlock('LINEで24時間相談でき、即日退職を目指しやすい。連絡を自分で取りたくない人、夜勤明けなどに相談したい人向け。スピード優先で動きたいときに適しています。', 'normal'),
    createBlock('即日対応の可否は、LINEの無料相談時にすぐ確認できます。', 'normal'),

    createBlock('弁護士法人ガイア法律事務所（医療介護の相談実績）', 'h3'),
    createBlock('医療・介護案件の経験が多く、就業規則や引き継ぎ手順にも配慮。LINE/メール相談で進めやすく、現場事情を理解してほしい人向け。', 'normal'),
    createBlock('現場の具体的な状況を含め、まずは無料相談で状況を伝えてみてください。', 'normal'),

    createBlock('推奨理由（公平・中立）', 'h4'),
    createLinkBlock('非弁リスクを避け、医療介護の相談実績と即日性・安心度を基準に3社を掲載。どれが自分に合うか、特徴から選びましょう。より詳しい比較は、', 'おすすめ退職代行3社比較', '/posts/comparison-of-three-resignation-agencies'),
    ...
    createBlock('「辞めてもいい」「限界なら逃げてもいい」。あなたの健康と人生が最優先です。無料相談も活用し、無理をせず次の一歩へ進んでください。安心して退職するための選択肢は、十分に用意されています。', 'normal'),
    ...
    // Disclaimer BEFORE Related Posts
    ...
    createBlock('あわせて読みたい', 'h3'),
    createLinkBlock('・', 'おすすめ退職代行3社比較', '/posts/comparison-of-three-resignation-agencies'),
    createLinkBlock('・', '看護助手向け転職サービス3社比較', '/posts/nursing-assistant-compare-services-perspective')
];
...
async function updateArticle() {
    ...
    if (res.ok) {
        console.log('Successfully updated article (v2):', json);
    } else {
        console.error('Failed to update article (v2):', json);
        process.exit(1);
    }
}
