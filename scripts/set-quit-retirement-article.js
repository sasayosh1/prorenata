...
const body = [
  block('イントロダクション', 'h2'),
  block('看護助手が「辞めたい」と感じるのは珍しくありません。この記事では、安心して退職できるように、選択肢と具体的な動き方を整理します。'),

  block('看護助手が辞めたいと感じるリアルな理由', 'h2'),
  block('・給料の低さ / 昇給の伸びにくさ'),
  block('・人手不足による業務量の多さ'),
  block('・人間関係のストレス'),
  block('・腰や膝にくる身体的負担'),
  block('・夜勤による生活リズムの乱れ'),
  ...
  block('・退職理由の整理（本音と伝える建前を分ける）'),
  block('・有給残日数の確認と消化計画'),
  block('・給与締め日、保険、離職票など手続きの確認'),
  block('・トラブル防止のため、やり取りをメモに残す'),
  block('・信頼できる人（同僚、家族、専門窓口）への相談'),

  block('看護助手が取れる3つの退職方法', 'h2'),
  block('自分で退職を伝える', 'h3'),
  block('費用ゼロ。関係が悪くなく、交渉に慣れている人向け。'),
  block('退職代行サービスを利用する', 'h3'),
  block('即日退職を目指しやすく、連絡・交渉を任せられる。上司と話すのが怖い、ハラスメントがある人向け。弁護士・労組運営を推奨。'),
  block('サポートを受けながら自分で伝える（セルフ退職）', 'h3'),
  block('テンプレや相談を受けつつ、自分名義で進める。費用を抑えたいが手順に不安がある人向け。'),
  block('比較の目安', 'h3'),
  ...
  block('・伝え方テンプレ: 「家庭の事情で○月末に退職を希望します。引き継ぎは文書で用意しますので日程をご相談させてください。」'),
  block('・本音を建前に変える: 人手不足→家庭都合・キャリア見直し。賃金不満→別分野で経験を積みたい。'),
  block('・引き止め回避: 退職日は確定ベースで伝え、延長は難しいと先に言う。引き継ぎ内容・期限をセットで提示。'),
  block('・スケジュール: 意思表明→退職日確定→引き継ぎ→有給消化→離職票・証明書受領'),

  block('退職代行サービスの安全性と選び方', 'h2'),
  block('・弁護士・労働組合を推奨: 法的交渉や書類対応を適法に行える'),
  block('・非弁業者のリスク: 不当請求、手続き不備、トラブル時に対応できない'),
  block('・医療現場で起こりがちなトラブル: 有休消化交渉、書類不備、引き継ぎ拒否など'),
  block('・LINE相談〜退職完了の流れ: 無料相談→申込・支払い→会社連絡→退職意思伝達→書類・精算確認'),

  block('安心して利用できる退職代行サービス比較（3社）', 'h2'),
  block('弁護士法人みやび（法的安心・全国対応）', 'h3'),
  block('料金: 16,500円（税込） / 特徴: 弁護士が直接対応。未払賃金・ハラスメント交渉も可能。医療・介護の実績多数。'),
  block('退職代行 即ヤメ（スピード・24時間相談）', 'h3'),
  block('料金: 15,000円（税込） / 特徴: LINEで24時間相談、即日退職を目指しやすい。連絡を自分で取りたくない人向け。'),
  block('弁護士法人ガイア法律事務所（医療介護の相談実績）', 'h3'),
  block('料金: 14,000円（税込） / 特徴: 医療・介護案件の経験が多い。就業規則や引き継ぎ手順にも配慮。'),
  block('推奨理由（公平・中立）', 'h3'),
  block('・非弁リスクを避け、医療介護の相談実績がある3社を掲載。'),
  block('・即日性、法的安心度、相談しやすさを基準に選定。'),
  ...
  block('「辞めてもいい」「限界なら逃げてもいい」。あなたの健康と人生が最優先です。無料相談も活用し、無理せず次のステップへ進んでください。')
]
